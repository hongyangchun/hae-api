...
> ⚠️ **判据是退出码，不是「有没有全部 ✅」。** 早期版本在「断言恰好全过 + 有请求丢包」时
> 会报「全部通过」并 `exit 0` —— 丢包会让卡片整张空掉（值 `--`、副标题「基准不足」），
> 而「今天的数据不该标日期」那条断言在**空卡片上恰好成立**，于是给出假绿灯，把网络问题
> 当成了代码没问题。现已两处修补：日结型卡片先断言「接口有数据 → 卡片必须有值」；
> 只要渲染期间有请求失败（`wkfail` 场景除外）一律 `exit 2`，哪怕断言全过。
> 另注：失败记录只写不删，「先失败后重试成功」会留下**假警告**，别被它反向误导。
