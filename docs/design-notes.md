...
三端 = `/dashboard`（网页）· SwiftBar 下拉 + 面板（macOS）· Quickshell 弹层（Omarchy）。
**只有一份数据源，却有四个渲染面**（`dashboard.js` / `macos/render.py` / `macos/panel.template.html` /
...
层级是倒置的。现在仪表盘首屏就是状态条（结论 + HRV/静息心率/睡眠 三个带基准的量）。
...
英文词语全部译中（`READY`→`可以练`、`7d base`→`7 日均`、`TRAINING`→`训练`、`Open panel`→`打开面板`）。
...
SwiftBar 的 webview 面板与下拉信息高度重叠，维护成本却是独立的。决定**保留但降级**：
只放下拉放不下的东西 —— 可视化趋势 + 一个跳去完整仪表盘的入口，**不做下拉的放大版**；
排版 token 与下拉共用（同序、同列宽、同语言、主题跟随系统）。
