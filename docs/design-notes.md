...
19. **派生指标的窗口默认值必须落在今天**（2026-09-17 修）：`vo2_max_est` 的窗口语义是
    ...
    **裸调用 `/api/query?name=vo2_max_est` 永远返回「参考最大心率不足，无法估算」**，
    ...
    不传 `from` 仍返回全部可算的点）。
20. **假绿灯：断言「恰好成立」比断言失败更危险**（2026-09-17 发现并修）。
    验收 `verify_dashboard_cards.mjs` 时遇到一轮：代理把 `query?name=sleep_analysis` 和
    `query?name=vo2_max_est` 两个请求丢了 → `P.sleepPts=[]` → `P.sleep=null` →
    **卡片整张空掉**（值列 `--`、副标题「基准不足」）。但日期后缀那条断言写的是
    `hasDate(c) === (数据日期 !== 今天)`，**空卡片上它恰好成立**（今天的数据确实不该标日期，
    空卡片确实没标）→ 脚本报「全部通过」，且当时还 `exit 0`。
    **等于把「网络丢了两个请求」判定成了「代码没问题」。** 这类漏洞比红灯危险得多 ——
    红灯会有人去看，假绿灯不会。
    两处修补（脚本内）：
    1. 日结型卡片**先断言「接口有数据 → 卡片必须有值」**（`hasVal === (day != null)`），
       空卡片再也过不去。断言数 15 → 19。
    2. 结论段改成**只要 `renderFailures` 非空（`wkfail` 场景除外）就 `exit 2`**，
       哪怕断言全过 —— 因为「恰好成立」这一事实无法从断言结果里分辨出来，只能靠网络状态兜。
    推论：**判据永远是退出码，不是「有没有全部 ✅」**；跑完还要看它有没有打那行
    「本轮结论不可信」。另有两处易误读：`renderFailures` 是 Map，**失败时写入、成功时不删除**，
    所以「先失败后重试成功」会留下**假警告**（看到警告 ≠ 这轮有问题）；
    `wkfail` 场景的失败是**有意注入**的，退出码判断必须排除它。
    同类风险已在 `verify_ingest.mjs` 上避开：它用 mock D1、**不联网**，不存在丢包导致的假绿灯。
