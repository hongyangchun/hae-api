...
> [!NOTE]
> 早期版本会在 `POST /api/data` 的响应里返回形如「「step_count」每天只有 1 个点：请在自动化里关闭 Aggregate Data」的 `warnings` —— 那是启发式误报（在「打开聚合」的推荐配置下必然触发，因为它无法区分「HAE 算错了」和「HAE 按推荐配置正常汇总」）。
> **该检测已从代码中移除**，`warnings` 字段保留为空数组以维持响应兼容。判断真实数据对不对，看下一节。
